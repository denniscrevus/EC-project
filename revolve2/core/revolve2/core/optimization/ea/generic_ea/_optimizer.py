...
class EAOptimizer(Process, Generic[Genotype, Fitness]):
    ...
    async def _evaluate_generation(
        ...
        db_id: DbId,
    ) -> (List[Fitness], List[Tuple], List[Fitness]):
        """
    ...
    __objective_serializer: Type[Serializer[float]]
    ...
    __objectives: Optional[List[Tuple]]
    __nr_joints: Optional[List[Fitness]]
    ...
    async def ainit_new(
        ...
        objective_serializer: Type[Serializer[float]],
        offspring_size: int,
        ...
        await self.__objective_serializer.create_tables(session)
        ...
        new_opt = DbEAOptimizer(
            ...
            dist_objective_table=self.__objective_serializer.identifying_table(),
            power_objective_table=self.__objective_serializer.identifying_table(),
            nr_of_joints_table=self.__objective_serializer.identifying_table(),
        )
        session.add(new_opt)
        await session.flush()
    ...
    async def run(self) -> None:
        ...
        if self.__latest_fitnesses is None:
            (self.__latest_fitnesses, self.__objectives, self.__nr_joints) = await self.__safe_evaluate_generation(
        ...
        while self.__safe_must_do_next_gen():
            ...
            self.__objectives = [
                self.__objectives[i] for i in old_survivors
            ] + survived_new_objectives

            self.__nr_joints = [
                self.__nr_joints[i] for i in old_survivors
            ] + survived_new_nr_joints
    ...
    async def __safe_evaluate_generation(
        ...
        (fitnesses, objectives, nr_joints) = await self._evaluate_generation(
            genotypes=genotypes,
            database=database,
            db_id=db_id,
        )
        ...
        return fitnesses, objectives, nr_joints
    ...
    async def __save_generation_using_session(
        ...
        initial_objectives: Optional[List[Tuple]],
        initial_nr_joints: Optional[List[Fitness]],
        new_individuals: List[_Individual[Genotype]],
        new_fitnesses: Optional[List[Fitness]],
        new_objectives: Optional[List[Tuple]],
        new_nr_joints: Optional[List[Fitness]]
    ) -> None:
        ...
        if initial_fitnesses is not None:
            ...
            dist_objective_ids = await self.__objective_serializer.to_database(
                session, [tuple[0] for tuple in initial_objectives]
            )

            power_objective_ids = await self.__objective_serializer.to_database(
                session, [tuple[1] for tuple in initial_objectives]
            )

            nr_joints_ids = await self.__fitness_serializer.to_database(
                session, initial_nr_joints
            )
            ...
            for i, row in enumerate(rows):
                ...
                row.distance_obj_id = dist_objective_ids[i]
                row.remaining_power_id = power_objective_ids[i]
                row.number_of_joints_id = nr_joints_ids[i]
        ...
        dist_objective_ids2: List[Optional[int]]

        if new_objectives is not None:
            dist_new_objectives = [float(obj_tuple[0]) for obj_tuple in new_objectives]

            dist_objective_ids2 = [
                obj
                for obj in await self.__objective_serializer.to_database(
                    session, dist_new_objectives
                )
            ]  # this extra comprehension is useless but it stops mypy from complaining
            assert len(dist_objective_ids2) == len(dist_new_objectives)
        else:
            dist_objective_ids2 = [None for _ in range(len(new_individuals))]

        power_objective_ids2: List[Optional[int]]

        if new_objectives is not None:
            power_new_objectives = [float(obj_tuple[1]) for obj_tuple in new_objectives]

            power_objective_ids2 = [
                obj
                for obj in await self.__objective_serializer.to_database(
                    session, power_new_objectives
                )
            ]  # this extra comprehension is useless but it stops mypy from complaining
            assert len(power_objective_ids2) == len(power_new_objectives)
        else:
            power_objective_ids2 = [None for _ in range(len(new_individuals))]

        nr_joints_ids2 : list[Optional[int]]

        if new_nr_joints is not None:
            nr_joints_ids2 = [
                nr_joints
                for nr_joints in await self.__fitness_serializer.to_database(
                    session, new_nr_joints
                )
            ]
            assert len(nr_joints_ids2) == len(new_nr_joints)
        else:
            nr_joints_ids2 = [None for _ in range(len(new_individuals))]
        ...
        session.add_all(
            [
                DbEAOptimizerIndividual(
                    ...
                    genotype_id=g_id,
                    fitness_id=f_id,
                    distance_obj_id=d_id,
                    remaining_power_id=p_id,
                    number_of_joints_id=j_id,
                )
                for i, g_id, f_id, d_id, p_id, j_id in zip(new_individuals, genotype_ids, fitness_ids2, dist_objective_ids2, power_objective_ids2, nr_joints_ids2)
            ]
        )
