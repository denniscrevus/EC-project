...
from revolve2.core.optimization.ea.generic_ea.population_management import NSGA2
...
class Optimizer(EAOptimizer[Genotype, float]):
    ...
    async def ainit_new(  # type: ignore # TODO for now ignoring mypy complaint about LSP problem, override parent's ainit
        ...
        await super().ainit_new(
            ...
            objective_serializer=FloatSerializer,
        ...
        self._best_individual = Individual(None, (Decimal(0), Decimal(0)))
    ...
    def _crossover(self, parents: List[Genotype]) -> Genotype:
        ...
        rng = self._rng.uniform(0.0, 1.0)

        if self._crossover_prob > rng:
            return crossover(parents[0], parents[1], self._rng)
        else:
            rng_parent = self._rng.uniform(0.0, 1.0)
            if rng_parent <= 0.5:
                return parents[0]
            else:
                return parents[1]
    ...
    def convert_fitness(self, ranking, genotype_id_to_obj):
        ...
        for individual in ranking:
            obj_to_find = individual.objectives
            idd_to_remove = None

            for (idd, obj) in genotype_id_to_obj.items():
                if obj_to_find == obj:
    ...
    def _on_generation_checkpoint(self, session: AsyncSession) -> None:
        session.add(
            DbOptimizerState(
                ...
                best_individual_distance=self._best_individual.objectives[0],
                best_individual_remaining_battery=self._best_individual.objectives[1]
            )
        )
...
class DbOptimizerState(DbBase):
    ...
    best_individual_distance = sqlalchemy.Column(sqlalchemy.DECIMAL, nullable=False)
    best_individual_remaining_battery = sqlalchemy.Column(sqlalchemy.DECIMAL, nullable=False)
